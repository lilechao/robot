...
class Application(Frame):
    ...
    def read_log(self):
        time.sleep(1)
        with open(Logger.logname, encoding='UTF-8') as f:
            f.seek(0,2)
            while True:
                # data = f.readlines()
    ...
    def create_main(self):
        ...
        self.account_balance = Label(self.frame_account, text='余额：', justify=LEFT, relief=FLAT, width=6, fg='grey',font=('宋体',8))
        self.account_balance.grid(row=1, column=0)
        self.account_balance.grid_propagate(0)

        self.account_balance2 = Label(self.frame_account, text='0', justify='left', anchor='w', relief=FLAT, width=12, fg='red',font=('宋体',8))
        self.account_balance2.grid(row=1, column=1, sticky='w')
        # self.account_balance.grid_propagate(0)

        self.label_import = Button(self.frame_account, text='设置', width=8, justify=RIGHT, height=1, command=self.setting,font=('宋体',8))
        ...
        self.label_import = Button(self.frame_account, text='私钥导入', width=8, justify=RIGHT, height=1, command=self.input_private_key,font=('宋体',8))
        ...
        self.swap_contracts_list1 = ttk.Combobox(self.frame_swap_1, width=16, justify=LEFT, textvariable=StringVar(), height=30, state='readonly')
        ...
        self.swap_contracts_btn1 = Button(self.frame_swap_1, text='导入合约', width=7, justify=RIGHT, height=1, command=self.import_contract1)
        ...
        self.swap_contracts_input1 = Entry(self.frame_swap_1, textvariable = self.swap_contracts_input1_value, borderwidth=1, width=28, validate='key', validatecommand=(self.validate1, '%P'), highlightthickness=0)
        self.swap_contracts_input1.grid(row=1, column=1, columnspan=2)

        self.swap_contracts_balance1 = Label(self.frame_swap_1, text='余额', justify='right', anchor='e', relief=FLAT, width=8, fg='grey',font=('宋体',8))
        ...
        self.swap_contracts_balance1_label = Label(self.frame_swap_1, textvariable=self.swap_contracts_balance1_label_value, justify='right', anchor='w', relief=FLAT, width=20, fg='grey',font=('宋体',8))
        ...
        #合约列表2
        self.swap_contracts_list2 = ttk.Combobox(self.frame_swap_2, width=16, justify=LEFT, textvariable=StringVar(), height=30, state="readonly")
        ...
        self.swap_contracts_btn2 = Button(self.frame_swap_2, text='导入合约', width=8, justify=RIGHT, height=1, command=self.import_contract2)
        ...
        self.swap_contracts_input2 = Entry(self.frame_swap_2, textvariable = self.swap_contracts_input2_value, borderwidth=1, width=28, validate='key', validatecommand=(self.validate2, '%P'), highlightthickness=0)
        self.swap_contracts_input2.grid(row=1, column=1, columnspan=2)

        self.swap_contracts_balance2 = Label(self.frame_swap_2, text='余额', justify='right', anchor='e', relief=FLAT, width=8, fg='grey',font=('宋体',8))
        ...
        self.swap_contracts_balance2_label = Label(self.frame_swap_2, textvariable=self.swap_contracts_balance2_label_value, justify='right', anchor='w', relief=FLAT, width=20, fg='grey',font=('宋体',8))
        ...
        self.swap_contracts_received = Label(self.frame_swap_2, text='最少获得', justify='right', anchor='e', relief=FLAT, width=8, fg='grey',font=('宋体',8))
        self.swap_contracts_received.grid(row=3, column=2)
        self.swap_contracts_received.grid_propagate(0)
        self.swap_contracts_received_label = Label(self.frame_swap_2, text='0', justify='right', anchor='w', relief=FLAT, width=20, fg='grey',font=('',8))
        self.swap_contracts_received_label.grid(row=3, column=1, sticky='w')
        self.swap_contracts_received_label.grid_propagate(0)

        #开始
        self.swap_contracts_btn = Button(self.frame_swap_3, text='开始', width=6, justify=RIGHT, height=1, command=self.swap, padx=20)
        ...
        self.frame_swap_log_text = Text(self.frame_swap_log, width=46, height=10, bg='#ececec', fg='grey', relief=FLAT, state=DISABLED, highlightthickness=0)
    ...
    def import_contract1(self):
        if self.import_contract == True:
            ...
            if input_diaglog.contract_info:
                self.swap_contracts_list1.current(input_diaglog.contract_info['index'])
            self.import_contract_finished()
    ...
    def import_contract2(self):
        if self.import_contract == True:
            ...
            self.master.wait_window(input_diaglog)
            if input_diaglog.contract_info:
                self.swap_contracts_list2.current(input_diaglog.contract_info['index'])
            self.import_contract_finished()
            # self.init_price2(2)
            self.calc_price_thread('init_price2', self.init_price2, 2)
        return

    def import_contract_finished(self):
        self.import_contract = True
        Logger.info('导入合约完成')
        return
